"""
FP8 Training Script for GPT-2
Based on train_improved.py with FP8/TransformerEngine support

This script uses NVIDIA's Transformer Engine for FP8 training on Blackwell GPUs.
All checkpointing and TensorBoard features from train_improved.py are preserved.

Default checkpoint directory: ./checkpoints_fp8
"""
...
# Try to import TransformerEngine for FP8 support
try:
    import transformer_engine.pytorch as te
    from transformer_engine.common import recipe
    FP8_AVAILABLE = True
    print("✓ TransformerEngine available - FP8 training enabled")
except ImportError:
    FP8_AVAILABLE = False
    print("⚠ TransformerEngine not available - falling back to bfloat16")
    print("  Install with: pip install git+https://github.com/NVIDIA/TransformerEngine.git")
...
class CheckpointManager:
    ...
    def save_checkpoint(self, 
        ...
        checkpoint = {
            ...
            'args': args_dict,
            'timestamp': time.time(),
            'precision': 'fp8' if args_dict.get('use_fp8', False) else 'bfloat16',  # Track precision type
        }
    ...
    def load_checkpoint(self, checkpoint_path, model, optimizer, train_loader, val_loader, device):
        ...
        if 'precision' in checkpoint:
            print(f"  - Precision: {checkpoint['precision']}")
        print(f"{'='*60}\n")
    ...
    def list_checkpoints(self):
        ...
        if latest_path.exists():
            ...
            precision = ckpt.get('precision', 'unknown')
            print(f"  latest: step={ckpt['step']}, val_loss={ckpt['val_loss']:.4f}, precision={precision}")
        ...
        if best_path.exists():
            ...
            precision = ckpt.get('precision', 'unknown')
            print(f"  best:   step={ckpt['step']}, val_loss={ckpt['val_loss']:.4f}, precision={precision}")
        ...
        if epoch_ckpts:
            ...
            for path in epoch_ckpts:
                ...
                precision = ckpt.get('precision', 'unknown')
                print(f"  {path.name}: step={ckpt['step']}, val_loss={ckpt['val_loss']:.4f}, precision={precision}")
...
class TrainerFP8:
    def __init__(
        ...
        if self.use_fp8 and self.master_process:
            print(f"\n{'='*60}")
            print("🚀 FP8 Training Enabled!")
            print(f"  FP8 Format: {fp8_recipe.fp8_format if fp8_recipe else 'default'}")
            print(f"  Using TransformerEngine for FP8 optimization")
            print(f"{'='*60}\n")
    ...
    def train(
        ...
        for step in range(start_step, max_steps):
            ...
            for mini_step in range(self.grad_accum_steps):
                ...
                # FORWARD PASS with FP8 or bfloat16
                if self.use_fp8:
                    # FP8 training with TransformerEngine
                    with te.fp8_autocast(enabled=True, fp8_recipe=self.fp8_recipe):
                        logits, loss = self.model(inp, tar)
                else:
                    # Standard bfloat16 training
            ...
            if self.master_process:
                precision_str = "FP8" if self.use_fp8 else "BF16"
                print(f'step {step:5d} | epoch {self.current_epoch} | loss: {batch_loss.item():.6f} | lr: {lr:.2e} | norm: {norm:.4f} | dt: {dt:.2f}ms | tok/sec: {tokens_per_sec:.2f} | {precision_str}')
...
def get_args():
    import argparse
    parser = argparse.ArgumentParser(description="FP8 Training Configuration for GPT-2")
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="./checkpoints_fp8/", help="directory to save checkpoints (default: checkpoints_fp8)")
    ...
    parser.add_argument("--use_fp8", action='store_true', help="enable FP8 training with TransformerEngine")
    parser.add_argument("--fp8_format", type=str, default="hybrid", choices=["hybrid", "e4m3", "e5m2"], 
                       help="FP8 format: hybrid (default), e4m3 (better for forward), e5m2 (better for gradients)")
    parser.add_argument("--fp8_margin", type=int, default=0, help="FP8 scaling margin")
    parser.add_argument("--fp8_interval", type=int, default=1, help="FP8 amax history interval")
...
def main():
    ...
    print("\n" + "="*60)
    print("FP8 Training Configuration:")
    print("="*60)
    ...
    if args.use_fp8 and not FP8_AVAILABLE:
        print("⚠️  WARNING: FP8 requested but TransformerEngine not available!")
        print("   Falling back to bfloat16 training.")
        print("   To enable FP8, install: pip install git+https://github.com/NVIDIA/TransformerEngine.git\n")
        args.use_fp8 = False
    ...
    # Setup FP8 recipe if FP8 is enabled
    fp8_recipe = None
    if args.use_fp8 and FP8_AVAILABLE:
        # Map string format to TransformerEngine format
        format_map = {
            'hybrid': recipe.Format.HYBRID,
            'e4m3': recipe.Format.E4M3,
            'e5m2': recipe.Format.E5M2
        }
        fp8_format = format_map.get(args.fp8_format, recipe.Format.HYBRID)
        ...
        if master_process:
            print(f"\n{'='*60}")
            print("FP8 Recipe Configuration:")
            print(f"  Format: {args.fp8_format}")
            print(f"  Margin: {args.fp8_margin}")
            print(f"  Interval: {args.fp8_interval}")
            print(f"  Amax history length: 1024")
            print(f"{'='*60}\n")
    ...
    if args.use_tensorboard and master_process:
        ...
        if args.run_name is None:
            ...
            precision_tag = "fp8" if args.use_fp8 else "bf16"
            run_name = f"gpt2_{precision_tag}_{timestamp}"
        else:
        ...
        # Log hyperparameters
        hparams_dict = {k: v for k, v in args_dict.items() if isinstance(v, (int, float, str, bool))}
        hparams_dict['precision'] = 'fp8' if args.use_fp8 else 'bfloat16'
        writer.add_hparams(
            hparams_dict,
    ...
    if master_process:
        ...
        if args.use_fp8:
            print(f"Training Mode: FP8 (TransformerEngine)")
        else:
            print(f"Training Mode: bfloat16 (standard)")
